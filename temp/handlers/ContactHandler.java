...
public class ContactHandler extends Handler<ContactDetail> {
    ...
    public ContactHandler(Settings settings) {
        super(settings,HandlerType.CONTACT, LocalDB.SortType.UPDATE_ORDER, FcEntity.SimpleFcSerializer.getSerializer(ContactDetail.class), ContactDetail.class, true, LocalDB.DbType.LEVEL_DB);
        ...
        this.myPubKey = KeyTools.priKeyToPubKey(Decryptor.decryptPriKey(myPriKeyCipher, symKey));
    ...
    protected void reloadRemovedItems(BufferedReader br) {
        Map<String, Long> removedItems = getAllFromMap(LocalDB.LOCAL_REMOVED_MAP, FcEntity.SimpleSerializer.getSerializer(Long.class));
    ...
    private String opContact(String contactId, List<String> contactIds, ContactOpData.Op op,  BufferedReader br) {
        ...
        if (op.equals(ContactOpData.Op.ADD) ) {
            ...
            CryptoDataByte cryptoDataByte = encryptor.encryptByAsyOneWay(JsonUtils.toJson(contactDetail).getBytes(), myPubKey);
        ...
        for (Contact contact : contactList) {
            ...
            if (contactDetail == null) {
                fakeContactCipherMap.put(contact.getId(),
                    DateUtils.longToTime(contact.getBirthTime(), DateUtils.LONG_FORMAT)+" "+contact.getCipher());
                continue;
            }
