...
public class CashHandler extends Handler<Cash> {
    ...
    public CashHandler(String myFid, String myPriKeyCipher, byte[] symKey,
        ...
        this.myFid = myFid;
        this.priKey = Decryptor.decryptPriKey(myPriKeyCipher,symKey);
        this.br =br;
        unsafeIdJumpNumMap=new HashMap<>();
        this.cashDB = new PersistentSequenceMap(myFid,null, IndicesNames.CASH, dbPath);
        freshCashDB();
    }
    ...
    public CashHandler(Settings settings){
        ...
        unsafeIdJumpNumMap=new HashMap<>();
        this.cashDB = new PersistentSequenceMap(settings.getMainFid(),null, IndicesNames.CASH, settings.getDbDir());
        freshCashDB();
    }
    ...
    public void show() {
        ...
        while(true) {
            ...
            if(!Inputer.askIfYes(br,"Continue?"))return;
    ...
    public void incomes() {
        ...
        fcdsl.addNewExcept().addNewTerms().addNewFields(FieldNames.ISSUER).addNewValues(myFid, Values.OP_RETURN);
        fcdsl.addSort(FieldNames.BIRTH_HEIGHT, Strings.DESC).addSort(FieldNames.ID, Strings.ASC);
    ...
    public void expense() {
        Fcdsl fcdsl = new Fcdsl();
        fcdsl.addNewQuery().addNewTerms().addNewFields(FieldNames.ISSUER).addNewValues(myFid);
        fcdsl.addNewExcept().addNewTerms().addNewFields(FieldNames.OWNER).addNewValues(myFid, Values.OP_RETURN);
        fcdsl.addSort(FieldNames.SPEND_HEIGHT, Strings.DESC).addSort(FieldNames.ID, Strings.ASC);
        fcdsl.addSize(BATCH_SIZE);
        List<String> last = new ArrayList<>();
        int count = 0;
        while(true){
   ...
   public void rearrange(List<Cash> validCashList) {
    String tx = null;
    double sumCoin = FchUtils.satoshiToCoin(Cash.sumCashValue(validCashList));
    ...
    public void carve(List<Cash> chosenCashes) {
        ...
        if(!Inputer.askIfYes(br, "Do you sure to carve the words on chain. It's irreversible. "))return;
    ...
    public String swapAll(List<Cash> cashList, String words) {
            ...
            long feeLong = TxCreator.calcFee(cashList.size(), 1, wordsLength, TxCreator.DEFAULT_FEE_RATE,false,null);
    ...
    public String batchMerge(List<Cash> cashList, double sumCoin, BufferedReader br) {
        ...
        double rest = NumberUtils.roundDouble8(sumCoin-amount*(mergeToCount-1));
        ...
        if(!Inputer.askIfYes(br, "Do it?"))return null;
        ...
        double restAmount = calcRestAmount(cashList.size(), cashAmountSum, paid, sendToList.size(), 0, TxCreator.DEFAULT_FEE_RATE, false, null);
        ...
        if(Inputer.askIfYes(br,"Send it?"))
    ...
    public String manualMerge(List<Cash> cashList, BufferedReader br) {
        ...
        double rest = calcRestAmount(cashList.size(), avaliable, 0, 0, 0, TxCreator.DEFAULT_FEE_RATE, false, null);
        ...
        while (rest > 0) {
            ...
            if (input.isEmpty() && sendToList.size() > 0) {
                rest = calcRestAmount(cashList.size(), avaliable, 0, sendToList.size(), 0, TxCreator.DEFAULT_FEE_RATE, false, null);
                if(rest<0){
                    System.out.println("The rest is not enough to pay the fee.");
                    return null;
                }
    ...
    public void sendTx(@Nullable List<Cash> cashList, @Nullable BufferedReader br){
       ...
       System.out.println("Balance: "+ FchUtils.satoshiToCoin(cid.getBalance())+".\nCashes:  "+ cid.getCash());
       ...
       String result = send(cashList, sum, cd, sendToList, msg,br);
        ...
        if(Hex.isHexString(result)) {
            ...
            if(br!=null && !Inputer.askIfYes(br,"Sign it?"))return;
            ...
            if(StringUtils.isBase64(txSigned)){
                txSigned = StringUtils.base64ToHex(txSigned);
    ...
    public String send(@Nullable List<Cash> cashList, @Nullable Double amount, @Nullable Long cd, @Nullable List<SendTo> sendToList, @Nullable String opReturn, @Nullable BufferedReader br){
        ...
        if(immatureRemoved){
            ...
            if(br!=null && !Inputer.askIfYes(br,"Continue?"))return null;
        }
        ...
        String unSignedTx = TxCreator.createUnsignedTxForCs(meetList, sendToList, opReturn==null ? null : opReturn.getBytes(), null, TxCreator.DEFAULT_FEE_RATE, FchMainNetwork.MAINNETWORK);
        if(unSignedTx==null)return null;

        String signedTx  = TxCreator.signRawTx(unSignedTx, priKey, FchMainNetwork.MAINNETWORK);
        if(signedTx==null){
    ...
    public double calcRestAmount(List<Cash> cashList, List<SendTo> sendToList, int opReturnSize, Double feeRate, boolean isMultiSign, P2SH p2sh){
        if(feeRate==null)feeRate = TxCreator.DEFAULT_FEE_RATE;
        int sendSize=0;
    ...
    public List<Cash> chooseCasheList(BufferedReader br) {
        ...
        while(true){
            ...
            if(!Inputer.askIfYes(br,"Choose more?"))return finalCashes;
        }
        return finalCashes;
    }
    ...
    public List<Cash> getCashesForSend(@Nullable Double amount, @Nullable Long cd, int outputNum, int opReturnLength){
        ...
        List<Cash> cashList = cashDB.getListFromEnd( null, Constants.DEFAULT_DISPLAY_LIST_SIZE, Cash::fromBytes);
        long spendValue = 0L;
        ...
        if (totalValue >= (spendValue + fee) && totalCd >= cd) {
            ...
            if (cashDB.size() > Constants.DEFAULT_DISPLAY_LIST_SIZE) {
    ...
    private void addNewCash(String txId, int index, double amount, int maxJumpNum) {
        ...
        cash.setId(Cash.makeCashId(txId, index));
    ...
    public void freshCashDB(){
        ...
        if(bestHeight>lastHeight){
            if(apipClient!=null){
                ...
                try{    
                    Cid cid = EsUtils.getById(esClient, IndicesNames.CID,myFid, Cid.class);
    ...
    private void freshCashFileMapByEs(Long lastHeight) {
        ...
        sb.size(Constants.DEFAULT_DISPLAY_LIST_SIZE);
        List<SortOptions> sortOptionsList = Sort.makeTwoFieldsSort(FieldNames.LAST_TIME, Strings.DESC,FieldNames.ID, Strings.ASC);
        ...
        while(true){
            ...
            try {
                ...
                if (newLast != null)
                    last = newLast;
                if(cashList.size()< Constants.DEFAULT_DISPLAY_LIST_SIZE)break;
            } catch (IOException e) {
                log.error("EsClient error:{}", e.getMessage());
            }
        }
    ...
    private void freshCashFileMapByApip(Long lastHeight) {
        ...
        fcdsl.addNewQuery().addNewRange().addNewFields(FieldNames.BIRTH_HEIGHT, FieldNames.SPEND_HEIGHT).addGt(String.valueOf(lastHeight));
        fcdsl.getQuery().addNewTerms().addNewFields(FieldNames.OWNER).addNewValues(myFid);
        while(true){
    ...
    public static List<Cash> getAllCashListByFids(List<String> fids, Boolean valid, Long sinceHeight, int size, ArrayList<Sort> sortList, List<String> last, ApipClient apipClient, NaSaRpcClient nasaClient, ElasticsearchClient esClient) {
        ...
        if (apipClient != null) {
            Fcdsl fcdsl = new Fcdsl();
            fcdsl.addSize(size);
            fcdsl.addNewQuery().addNewTerms().addNewFields(FieldNames.OWNER).addNewValues(fids.toArray(new String[0]));
            if (valid != null) {
                fcdsl.getQuery().addNewTerms().addNewFields(FieldNames.VALID).addNewValues(valid.toString());
            }
            if (sinceHeight != null) {
    ...
    public void freshValidCashes(){
        ...
        if(apipClient!=null){
            ...
            ReplyBody replier = getAllCashListFromEs(new ArrayList<>(Arrays.asList(myFid)), true, null, Constants.DEFAULT_DISPLAY_LIST_SIZE, null, null, esClient);
            if(replier.getCode()!=0){
    ...
    public void updateCashesIfOverJumped(){
        ...
        if(maxJumpNum.equals(Constants.MAX_JUMP_NUM)){
    ...
    public List<Cash> getAllCashList(String fid, Boolean valid, ArrayList<Sort> sortList, List<String> last, BufferedReader br) {
        ...
        if (this.apipClient != null) {
            do {
                List<Cash> newCashList = getCashListFromApip(fid, valid, Constants.DEFAULT_DISPLAY_LIST_SIZE, sortList, last, null, apipClient);
                if (newCashList == null) {
                ...
                if(br!=null){
                    ...
                    if(!Inputer.askIfYes(br,"Do you want to continue to get cashes?")){
                        break;
                    }
            ...
            do {
                replyBody = getCashListFromEs(new ArrayList<>(Arrays.asList(fid)), valid, null, Constants.DEFAULT_DISPLAY_LIST_SIZE, sortList, last, esClient);
    ...
    public static ReplyBody getCashListFromEs(List<String> fids, Boolean valid, Long afterHeight, int size,
        ...
        else sb.size(Constants.DEFAULT_DISPLAY_LIST_SIZE);
        if (sortList != null && !sortList.isEmpty()) {
            List<SortOptions> sortOptionslist = Sort.getSortList(sortList);
            sb.sort(sortOptionslist);
        } else {
            List<SortOptions> sortOptionsList = Sort.makeTwoFieldsSort(FieldNames.BIRTH_HEIGHT, Strings.DESC, FieldNames.ID, Strings.ASC);
            sb.sort(sortOptionsList);
        }
        if (last != null && !last.isEmpty())
            sb.searchAfter(last);
        ...
        if (valid!=null ){
            if(valid)queryList.add(new Query(new TermQuery.Builder().field(FieldNames.VALID).value(Values.TRUE).build()));
            else queryList.add(new Query(new TermQuery.Builder().field(FieldNames.VALID).value(Values.FALSE).build()));
        }
    ...
    public static List<Cash> checkUnconfirmedNewCashesByJedis(String fid,Jedis jedis){
        Gson gson = new Gson();
        List<Cash> cashList = new ArrayList<>();
        String cashIdListStr = jedis.hget(fid, FieldNames.NEW_CASHES);
    ...
    private List<Cash> pullAllValidFromApip(@Nullable BufferedReader br, boolean choose, Long sinceHeight) {
         ...
         fcdsl.addSort(FieldNames.BIRTH_HEIGHT, Strings.DESC).addSort(FieldNames.BIRTH_TX_INDEX, Strings.ASC).addSort(FieldNames.BIRTH_INDEX, Strings.DESC);
         fcdsl.addSize(BATCH_SIZE);
         List<String> last = null;
         ...
         while(true){
             ...
             if(choose && br!=null){
                 ...
                 if(Inputer.askIfYes(br, "Continue to pull more cashes?")){
                     totalDisplayed += subCashList.size();
                     if(got<BATCH_SIZE)break;
                     if(finalCashes.size()> Constants.MAX_CASH_SIZE){
                         System.out.println("Warning: You have pulled more than "+ Constants.MAX_CASH_SIZE+" cashes. It's hard to deal with so many cashes.");
                         if(Inputer.askIfYes(br, "Continue?"))return finalCashes;
                     }
                     continue;
                 }else{
             ...
             if(finalCashes.size()> Constants.MAX_CASH_SIZE){
                 ...
                 if(br!=null && !Inputer.askIfYes(br, "Continue?"))return finalCashes;
             }
         }
         return finalCashes;
     }
    ...
    public static Boolean isImmature(Cash cash, Long bestHeight) {
        ...
        if(! Constants.COINBASE.equals(cash.getIssuer()))return false;

        if(cash.getOwner().equals(Constants.FUND_FID)){
            if( (bestHeight - cash.getBirthHeight()) < Constants.FUND_MATURE_DAYS_INT)return true;
        }

        return bestHeight - cash.getBirthHeight() < Constants.MINE_MATURE_DAYS_INT;
    }
    ...
    public static SearchResult<Cash> getValidCashes(String myFid, Long amount, Long cd, Long sinceHeight, int outputSize, int msgSize, ApipClient apipClient, ElasticsearchClient esClient, MempoolHandler mempoolHandler) {
        ...
        if(apipClient!=null){
            Fcdsl fcdsl = new Fcdsl();
            if(myFid!=null)fcdsl.addNewQuery().addNewTerms().addNewFields(FieldNames.OWNER).addNewValues(myFid);
            fcdsl.addNewFilter().addNewTerms().addNewFields(FieldNames.VALID).addNewValues(Values.TRUE);
            if(sinceHeight!=null)fcdsl.getQuery().addNewRange().addNewFields(FieldNames.BIRTH_HEIGHT).addGt(String.valueOf(sinceHeight));
            fcdsl.addSize(Constants.MAX_CASH_SIZE);
            fcdsl.addSort(FieldNames.CD, Strings.ASC).addSort(FieldNames.ID, Strings.ASC);
            ...
            try {
                ...
                TermQuery ownerQuery = new TermQuery.Builder().field(FieldNames.OWNER).value(myFid).build();
    ...
    public static String carve(String opReturn, Long cd, byte[] priKey, ApipClient apipClient) {
        ...
        Transaction unSignedTxBytes = TxCreator.createUnsignedTx(
            ...
            opReturn,
            null, 
            TxCreator.DEFAULT_FEE_RATE, null, FchMainNetwork.MAINNETWORK
        );
