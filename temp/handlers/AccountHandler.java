...
public class AccountHandler extends Handler<FcEntity> {
    ...
    public void menu(BufferedReader br, boolean withSettings) {
        ...
        menu.add("Show Income List", () -> {
            Integer incomeSize = Inputer.inputInteger(br, "Enter number of records to show: ", 1, Constants.DEFAULT_DISPLAY_LIST_SIZE);
            if(incomeSize==null)return;
            showIncomeList(incomeSize, br);
        });
        
        menu.add("Show Expense List", () -> {
            Integer expenseSize = Inputer.inputInteger(br, "Enter number of records to show: ", 1, Constants.DEFAULT_DISPLAY_LIST_SIZE);
            ...
            while(true){
                if(apipClient!=null){
                    ...
                    if((lastIncome==null|| lastIncome.isEmpty()) && startHeight!=null) {
                        fcdsl.getQuery().addNewRange().addNewFields(FieldNames.BIRTH_HEIGHT).addGt(startHeight);
                    }
                    fcdsl.addSort(FieldNames.BIRTH_HEIGHT, Values.ASC).addSort(FieldNames.ID, Values.ASC);
                    ...
                    SearchRequest.Builder searchBuilder = new SearchRequest.Builder()
                        ...
                        .query(q -> q.bool(boolQueryBuilder.build()))
                        .sort(soList)
                        .size(Constants.DEFAULT_DISPLAY_LIST_SIZE);
                ...
                if(newCashes.size() < Constants.DEFAULT_DISPLAY_LIST_SIZE){
                    break;
                }
            }
    ...
    private void checkIncomeVia(List<Cash> newCashes) {
        ...
        for(Map.Entry<String, OpReturn> entry:opReturnMap.entrySet()){
            ...
            if(opReturnData.containsKey(Strings.VIA)){
                ...
                for(Cash cash:matchingCashes){
                    addViaBalance(cash.getIssuer(),cash.getValue(),opReturnData.get(Strings.VIA));
                }
            }
        }
    }
    ...
    public static Long updateUserBalance(String sid, String uid, Long value, JedisPool jedisPool) {
        ...
        try (Jedis jedis = jedisPool.getResource()) {
            String key = IdNameUtils.makeKeyName(null, sid, FieldNames.BALANCE, null);
