...
public class TeamManager extends Manager<Team> {
    ...
    // 3. Constructor
    public TeamManager(Settings settings){
        super(settings, ManagerType.TEAM, LocalDB.SortType.UPDATE_ORDER, Team.class, true, true);
    ...
    public String createTeam(byte[] priKey, String offLineFid, List<Cash> sendToList,
            String stdName, String consensusId, Map<String, String> localNames, String[] waiters, 
            String[] accounts, String desc, Map<String, String> home, ApipClient apipClient, NaSaRpcClient nasaClient) {
        TeamOpData data = TeamOpData.makeCreate(stdName, consensusId, localNames, waiters, accounts, desc, home);
    ...
    private void handleCreateTeam(byte[] priKey, BufferedReader br) {
        ...
        Map<String, String> localNames = Inputer.inputStringStringMap(br,
        ...
        if(!Inputer.askIfYes(br, "Name:" + stdName + "\nConsensus ID:" + consensusId + 
                "\nDescription:" + desc + "\nCreate it?")) return;
        String createResult = createTeam(priKey, myFid, null, stdName, consensusId,
                localNames.isEmpty() ? null : localNames, waiters, accounts, desc, null, apipClient, null);
