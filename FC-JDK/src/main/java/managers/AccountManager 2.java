...
public class AccountManager extends Manager<FcEntity> {
    ...
    public AccountManager(Settings settings){
        ...
        String priceStr = service.getPricePerKB();
        if(priceStr!=null) {
            double price = Double.parseDouble(priceStr);
            this.priceBase = utils.FchUtils.coinToSatoshi(price);
        }else this.priceBase = 0L;
        String orderViaShareStr = service.getOrderViaShare();
        orderViaShare = orderViaShareStr != null ? Double.parseDouble(orderViaShareStr) : 0;
        String consumeViaShareStr = service.getConsumeViaShare();
    ...
    /**
     * Apply a delta to the user's balance atomically.
     *
     * This runs on every billed API reply, so concurrent requests from one FID are normal. A
     * read-modify-write over HGET/HSET loses debits under that concurrency; HINCRBY applies the
     * delta server-side in one operation and returns the resulting value.
     */
    private long updateUserBalanceInRedis(String userFid, Long value, Jedis jedis) {
        long newBalance = jedis.hincrBy(redisKeyUserBalance, userFid, value);
        if (newBalance < -minCredit) {
            // The delta is already applied; updateIncome() may credit the account, so re-read to
            // report the current value rather than applying the delta a second time.
            updateIncome();
            String valueStr = jedis.hget(redisKeyUserBalance, userFid);
            if (valueStr != null) {
                try {
                    newBalance = Long.parseLong(valueStr);
    ...
    private synchronized long updateUserBalanceInLocal(String userFid, Long value) {
    ...
    public List<Income> updateIncome() {
        ...
        while(true){
            if(apipClient!=null){
                ...
                if (!response.hits().hits().isEmpty()) {
                    lastIncome = EsUtils.toStringList(response.hits().hits().get(response.hits().hits().size() - 1).sort());
        ...
        while(true) {
            ...
            if(apipClient!=null){
                ...
                try {
                    ...
                    if (!response.hits().hits().isEmpty()) {
                        lastExpense = EsUtils.toStringList(response.hits().hits().get(response.hits().hits().size() - 1).sort());
