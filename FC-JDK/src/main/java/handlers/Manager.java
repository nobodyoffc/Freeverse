...
//    protected volatile AtomicBoolean isRunning = new AtomicBoolean(false);
    ...
    private final Boolean opOnChain;
        ...
        public Manager(ManagerType managerType){
            ...
            this.itemClass = null;
            this.apipClient = null;
            this.itemName = managerType.name().toLowerCase();
            this.cashHandler = null;
            this.settings = null;
            this.opOnChain=false;
        }
    public Manager(Settings settings, ManagerType managerType) {
        this(settings, managerType,null, null, false, false);
    }
    public Manager(Settings settings, ManagerType managerType, LocalDB.SortType sortType, Class<T> itemClass, boolean withLocalDB, Boolean opOnChain) {
        ...
        this.itemName = managerType.toString();

        log.info("Creating {} Handler: {}", itemName, this.mainFid);
    ...
    public void menu(BufferedReader br, boolean withSettings) {
        Menu menu = new Menu(managerType.toString(), this::close);
...
//    }
    ...
    protected String carve(String opReturnStr, long cd, BufferedReader br) {
        ...
        else if(apipClient!=null) result = CashManager.carve(opReturnStr, cd, prikey, apipClient);
        else return null;
    ...
    public ManagerType getHandlerType() {
        return managerType;
    }
    ...
    public void close() {
        if (localDB != null) {
            try {
                ...
                log.error("Error closing LevelDB for handler {}: {}", managerType, e.getMessage(), e);
                throw new RuntimeException("Failed to close LevelDB for handler " + managerType, e);
            }
        }
    }
    ...
    protected LocalDB<T> createLocalDB(LocalDB.SortType sortType,
        ...
        return switch (dbType) {
            case EASY_DB -> new EasyDB<>(sortType, entityClass);
            default -> new LevelDB<>(sortType, entityClass);
        };
    }
