    ...
    public byte[] getData() {
        if (data == null && dataFile != null) {
            ...
            if (dataFileLength > Integer.MAX_VALUE) {
                throw new IllegalStateException("File-backed notify too large to materialise: " + dataFileLength);
            }
            ...
            try (InputStream in = openData()) {
                ...
                while (off < buf.length) {
                    ...
                    if (n < 0) throw new IOException("Truncated file-backed notify data at " + off
                            + " of " + buf.length);
                    off += n;
                }
            } catch (IOException e) {
