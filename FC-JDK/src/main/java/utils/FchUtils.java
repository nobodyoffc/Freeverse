...
public class FchUtils {
    ...
    public static void updateCidNumbers(ElasticsearchClient esClient,List<Freer> cidList) throws IOException {
        ...
        applyAggsToCids(aggsMaps, cidList);
    }

    /**
     * Apply pre-aggregated TXO/STXO/UTXO totals onto the given Freer records.
     *
     * Split out from {@link #updateCidNumbers} so the arithmetic can be exercised without an
     * Elasticsearch client.
     */
    public static void applyAggsToCids(Map<String, Map<String, Long>> aggsMaps, List<Freer> cidList) {
        ...
        for(Freer cid:cidList) {
            ...
            String addr = cid.getId();
            // INCOME is the total received (TXO_SUM), not the total spent (STXO_SUM).
            Long income = txoSumMap.get(addr);
            cid.setIncome(income != null ? income : 0L);

            if(utxoSumMap.get(addr)!=null) {
                Long utxoCount = utxoCountMap.get(addr);
                cid.setBalance(utxoSumMap.get(addr));
                cid.setCash(utxoCount != null ? utxoCount : 0L);
            }else {
                cid.setBalance(0L);
                cid.setCash(0L);
            }
