...
//    }
    ...
    /**
     * Deterministic sort used to page through history indices: height, then index within the
     * block, then the document id as a unique tiebreaker. Every *_HISTORY mapping declares all
     * three, and `id` is the txid, so this is a total order and search_after cannot skip or
     * repeat a document.
     */
    private static List<SortOptions> historyScanSort() {
        List<SortOptions> soList = new ArrayList<>();
        soList.add(SortOptions.of(s -> s.field(FieldSort.of(f -> f.field(HEIGHT).order(SortOrder.Asc)))));
        soList.add(SortOptions.of(s -> s.field(FieldSort.of(f -> f.field(INDEX).order(SortOrder.Asc)))));
        soList.add(SortOptions.of(s -> s.field(FieldSort.of(f -> f.field(ID).order(SortOrder.Asc)))));
        return soList;
    }

    /**
     * Return every hit in `index` whose `heightField` is greater than `height`.
     *
     * Rollback correctness depends on seeing the complete set of affected histories: a plain
     * search returns only Elasticsearch's default 10 hits, which silently reverts a fraction of
     * a reorganization. This pages with search_after over a total order until exhausted.
     */
    public static <T> List<Hit<T>> scanHitsAboveHeight(ElasticsearchClient esClient, String index,
                                                       String heightField, long height, Class<T> clazz) throws IOException {
        return scanHitsAboveHeight(esClient, index, heightField, height, false, clazz);
    }

    /**
     * As above; `inclusive` selects gte instead of gt for the height bound.
     */
    public static <T> List<Hit<T>> scanHitsAboveHeight(ElasticsearchClient esClient, String index,
                                                       String heightField, long height, boolean inclusive,
                                                       Class<T> clazz) throws IOException {
        List<SortOptions> soList = historyScanSort();
        List<Hit<T>> allHits = new ArrayList<>();
        List<FieldValue> searchAfter = null;

        while (true) {
            List<FieldValue> lastSort = searchAfter;
            SearchResponse<T> response = esClient.search(s -> {
                s.index(index)
                        .size(READ_MAX)
                        .sort(soList)
                        .trackTotalHits(t -> t.enabled(false))
                        .query(q -> q.range(r -> inclusive
                                ? r.field(heightField).gte(JsonData.of(height))
                                : r.field(heightField).gt(JsonData.of(height))));
    ...
    /**
     * Fetch every history document referencing any of `itemIdList`, matched either by
     * `termsField1` or by the optional multi-id `termField2` (e.g. pid / pids).
     *
     * Both the first page and every continuation page must run the SAME query: an earlier
     * version dropped the termField2 clause after page one, so histories matched only by the
    ...
    public static <T> List<T> getHistsForReparse(ElasticsearchClient esClient, String index, String termsField1, String termField2, ArrayList<String> itemIdList, Class<T> clazz) throws ElasticsearchException, IOException {
        ...
        for (String v : itemIdList) {
            if (v != null) itemValueList.add(FieldValue.of(v));
        }
        if (itemValueList.isEmpty()) return historyList;
        ...
        while (true) {
            List<FieldValue> lastSort = searchAfter;
            SearchResponse<T> result = esClient.search(s -> {
                s.index(index)
                        .size(READ_MAX)
                        .sort(soList)
                        .trackTotalHits(t -> t.enabled(false))
            ...
            List<Hit<T>> hits = result.hits().hits();
            if (hits.isEmpty()) break;

            for (Hit<T> hit : hits) {
                if (hit.source() != null) historyList.add(hit.source());
            }
