...
public class Freer extends FcSubject {
    ...
    public static List<Freer> showCidList(String title, List<Freer> list, Integer maxFieldWidth, boolean choose, BufferedReader br) {
        return Shower.showOrChooseListInPages("FID Info", list, DEFAULT_PAGE_SIZE, null, choose, Freer.class, br);
    }
    ...
    public void makeAddresses(String pk) {
        if(this.pubkey==null) {
            if(pk.length()!=64)
                pk = KeyTools.getPubkey33(pk);
            setPubkey(pk);
        }
        if(this.btcAddr==null) setBtcAddr(KeyTools.pubkeyToBtcAddr(pk));
        if(this.ethAddr==null) setEthAddr(KeyTools.pubkeyToEthAddr(pk));
        if(this.bchAddr==null) setBchAddr(KeyTools.pubkeyToBchBesh32Addr(pk));
        if(this.ltcAddr==null)  setLtcAddr(KeyTools.pubkeyToLtcAddr(pk));
        if(this.dogeAddr==null) setDogeAddr(KeyTools.pubkeyToDogeAddr(pk));
        if(this.trxAddr==null) setTrxAddr(KeyTools.pubkeyToTrxAddr(pk));
    }
