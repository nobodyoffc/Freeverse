...
//        transaction.setLockTime(nLockTime);
    ...
    public static String decodeTxFch(byte[] rawTxBytes, MainNetParams mainnetwork) {
        ...
        Transaction transaction;
            ...
            try (ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(rawTxBytes)) {
                // read() returns 0-255 (or -1 at EOF) while OFF_LINE_TX_START_FLAG is a byte that
                // widens to -1, so comparing them directly never matched a real 0xFF prefix and
                // instead matched empty input. Compare against the unsigned value.
                int flag = byteArrayInputStream.read();
                if(flag == (OFF_LINE_TX_START_FLAG & 0xFF)) {
                    byte[] b2 = new byte[2];
    ...
    /**
     * Assemble signature blobs collected out of band into one signed multisig transaction.
     *
     * Every blob must describe the SAME transaction and the SAME multisig address. Previously the
     * first blob's rawTx and multisig won and later ones were discarded unchecked, so a co-signer
     * who signed a different transaction still had their signatures assembled onto this one; and
     * signatures were merged by FID without checking that the FID belongs to the multisig at all.
     * Both are now rejected rather than silently producing a wrong transaction.
     */
    public static String buildSignedTx(String[] signedData, MainNetParams mainnetwork) {
        ...
        for (String dataJson : signedData) {
            RawTxInfo multiSignData;
            try {
                multiSignData = RawTxInfo.fromJson(dataJson, RawTxInfo.class);
            } catch (Exception e) {
                log.error("Failed to parse signed data blob: {}", e.getMessage());
                return null;
            }
            if (multiSignData == null) {
                log.error("Signed data blob parsed to null.");
                return null;
            }

            Multisig blobMultisig = multiSignData.getSenderMultisig();
            if (blobMultisig != null) {
                if (multisig == null) multisig = blobMultisig;
                else if (!sameMultisig(multisig, blobMultisig)) {
                    log.error("Signed data blobs refer to different multisig addresses.");
                    return null;
                }
            }

            byte[] blobRawTx = multiSignData.getRawTx();
            if (blobRawTx != null && blobRawTx.length > 0) {
                if (rawTx == null) rawTx = blobRawTx;
                else if (!Arrays.equals(rawTx, blobRawTx)) {
                    log.error("Signed data blobs refer to different raw transactions.");
                    return null;
                }
            }

            Map<String, List<String>> blobSigs = multiSignData.getFidSigMap();
            if (blobSigs == null) continue;
            for (Map.Entry<String, List<String>> entry : blobSigs.entrySet()) {
                List<String> existing = fidSigListMap.get(entry.getKey());
                if (existing != null && !existing.equals(entry.getValue())) {
                    log.error("Conflicting signatures supplied for {}.", entry.getKey());
                    return null;
                }
                fidSigListMap.put(entry.getKey(), entry.getValue());
            }
        }
        if (rawTx == null || multisig == null) return null;

        List<String> memberFids = multisig.getFids();
        if (memberFids == null || memberFids.isEmpty()) {
            log.error("Multisig has no member fids.");
            return null;
        }
        for (String fid : fidSigListMap.keySet()) {
            if (!memberFids.contains(fid)) {
                log.error("Signature supplied for {}, which is not a member of this multisig.", fid);
                return null;
            }
        }

        Map<String, List<byte[]>> sigBytesListMap = getStringListMap(fidSigListMap);
        if (multisig.getM() != null && sigBytesListMap.size() < multisig.getM()) {
            log.error("Only {} of the required {} signers supplied signatures.",
                    sigBytesListMap.size(), multisig.getM());
            return null;
        }
    ...
    /** Two multisig descriptors are the same iff they redeem with the same script. */
    private static boolean sameMultisig(Multisig a, Multisig b) {
        if (a.getRedeemScript() != null && b.getRedeemScript() != null)
            return a.getRedeemScript().equals(b.getRedeemScript());
