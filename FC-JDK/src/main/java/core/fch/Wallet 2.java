...
public class Wallet {
    ...
    public static String makeTxByEs(BufferedReader br, byte[] priKey, List<Cash> sendToList, String opReturnStr, long cd, int maxCashes, ElasticsearchClient esClient) {
        return makeTx(br, priKey, null, sendToList, opReturnStr, cd, maxCashes, null, esClient);
    }

    public static String makeTxForCs(BufferedReader br, String fid, List<Cash> sendToList, String opReturnStr, long cd, int maxCashes, ApipClient apipClient) {
        return makeTx(br, null, fid, sendToList, opReturnStr, cd, maxCashes, apipClient, null);
    }

    public static String makeTx(BufferedReader br, byte[] priKey, String fidForOfflineSign, List<Cash> sendToList, String opReturnStr, long cd, int maxCashes, ApipClient apipClient, ElasticsearchClient esClient){
    ...
    public static String makeTx(byte[] priKey, String fidForOfflineSign, List<Cash> sendToList, String opReturnStr, long cd, int maxCashes, ApipClient apipClient, ElasticsearchClient esClient, NaSaRpcClient nasaClient) {
        String fid;
        ...
        // The requested output total, in satoshi. `sum` is in coins because Cash.getAmount()
        // converts; the selection loop below compares against satoshi values, so it has to be
        // converted here. This used to be a `long amount = 0` that was never assigned, which
        // reduced the whole selection condition to "cover the fee".
        long amount = utils.FchUtils.coinToSatoshi(sum);
...
//            return makeTxForOfflineSign(sendToList, opReturnStr, cashList);
    ...
    public static String makeTxForOfflineSign(List<Cash> sendToList, String opReturn, List<Cash> meetList) {
    ...
    public static final int MERGE_BATCH_SIZE = 100;

    /**
     * Merge a cash list in batches, each batch collapsing into a single output.
     *
     * The batch count is size/BATCH_SIZE rounded up. It used to be `size % 100`, which for a list
     * of exactly 200 gave zero extra iterations and silently dropped half the input; and each
     * batch was passed issueNum = 0, which divided by zero in the overload below.
     */
    public static void mergeCashList(List<Cash> cashList, byte[] priKey, ApipClient apipClient, NaSaRpcClient nasaClient) {
        ...
        for (int start = 0; start < size; start += MERGE_BATCH_SIZE) {
            List<Cash> subCashList = new ArrayList<>(cashList.subList(start, Math.min(start + MERGE_BATCH_SIZE, size)));
            mergeCashList(subCashList, 1, priKey, apipClient, nasaClient);
        }
    }
    ...
    public static String mergeCashList(List<Cash> cashList, int issueNum, byte[] priKey, ApipClient apipClient, NaSaRpcClient nasaClient) {
        ...
        if (issueNum <= 0) {
            System.out.println("The number of outputs must be at least 1.");
            return null;
        }
