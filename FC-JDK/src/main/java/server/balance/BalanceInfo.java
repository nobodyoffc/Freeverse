...
//    public static final String BALANCE_BACKUP_JSON = "balanceBackup.json";
    ...
    public static void deleteOldBalance(String sid,ElasticsearchClient esClient) {
        String index = addSidBriefToName(sid, FieldNames.BALANCE);
...
//            }
    ...
    public static void recoverBalanceToRedis(String sid, BalanceInfo balanceInfo, Jedis jedis) {
        ...
        for (String id : balanceMap.keySet()) {
            jedis.hset(Settings.addSidBriefToName(sid, FieldNames.BALANCE), id, balanceMap.get(id));
        }
        for (String id : orderViaMap.keySet()) {
            jedis.hset(Settings.addSidBriefToName(sid, FieldNames.ORDER_VIA), id, orderViaMap.get(id));
        }
        for (String id : consumeViaMap.keySet()) {
            jedis.hset(Settings.addSidBriefToName(sid, FieldNames.CONSUME_VIA), id, consumeViaMap.get(id));
        }
        ...
        jedis.hset(Settings.addSidBriefToName(sid, FieldNames.CONSUME_VIA), BEST_HEIGHT, String.valueOf(balanceInfo.getBestHeight()));
    ...
    public static void backupBalance(String sid,ElasticsearchClient esClient,JedisPool jedisPool)  {
        try(Jedis jedis0Common = jedisPool.getResource()) {
            Map<String, String> balanceMap = jedis0Common.hgetAll(Settings.addSidBriefToName(sid, FieldNames.BALANCE));
            Map<String, String> consumeViaMap = jedis0Common.hgetAll(Settings.addSidBriefToName(sid, FieldNames.CONSUME_VIA));
            Map<String, String> orderViaMap = jedis0Common.hgetAll(Settings.addSidBriefToName(sid, FieldNames.ORDER_VIA));
    ...
    private static void backupBalanceToEx(String sid,ElasticsearchClient esClient, BalanceInfo balanceInfo, long bestHeight) throws IOException {
        String index = addSidBriefToName(sid, FieldNames.BALANCE).toLowerCase();
