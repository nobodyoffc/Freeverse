...
public class StartDiskClient {
    ...
    private static void signInEcc(byte[] symKey) {
        Session session = diskClient.signInEcc(settings.getApiAccount(ServiceType.DISK), RequestBody.SignInMode.NORMAL, symKey);
    ...
    private static void signIn(byte[] symKey) {
        Session session = diskClient.signIn(settings.getApiAccount(ServiceType.DISK), RequestBody.SignInMode.NORMAL,symKey);
        JsonTools.printJson(session);
        saveConfig();
        Menu.anyKeyToContinue(br);
    }
