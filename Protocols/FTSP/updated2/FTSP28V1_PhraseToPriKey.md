...
- **Do not confuse with `PasswordToSymkey` (FTSP25).** FTSP25 derives a *symmetric* key that is IV-bound and intentionally fast (two SHA-256 passes) for encrypting local data. PhraseToPriKey derives an *asymmetric* private key and is intentionally slow. They are different primitives with different threat models and MUST NOT be substituted for one another.
...
|[FTSP25 PasswordToSymkey](FTSP25V1_PasswordToSymkey.md)|Shares the `Argon2id@No1_NrC7` primitive lineage but targets IV-bound *symmetric* keys for `EncryptType.Password`, not asymmetric private keys.|
