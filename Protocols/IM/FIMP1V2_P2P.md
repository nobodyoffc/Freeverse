...
Throughout this section, `content` and `data` name the two **sections of the body** (FIMP0V2 §Body Framing), not envelope fields. Where the body is sealed (§7), they describe the plaintext *inside* the seal.
...
This document defines version 2 of the P2P mode (FIMP1V2), which accompanies the version 2 envelope of FIMP0V2 and does not interoperate with version 1.

Changes from FIMP1V1:
...
- **FIMP2V2** -- Room mode (uses P2P as the transport for SYMKEY pushes and ROOM_INFO invites).
- **FIMP4V2** -- Team mode (uses P2P as the transport for SYMKEY pushes).
