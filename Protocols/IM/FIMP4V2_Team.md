...
This document defines version 2 of the Team mode (FIMP4V2), which accompanies the version 2 envelope of FIMP0V2 and does not interoperate with version 1.

Changes from FIMP4V1:
...
- **FIMP1V2** -- P2P mode (used as the carrier for SYMKEY, MEMBERS, HISTORY responses, REQUESTs, and the source of their sealing rules).
- **FIMP2V2** -- Room mode (analogous closed-group semantics, but with local non-on-chain membership).
- **FIMP3V2** -- Square mode (open on-chain membership, no IM-layer sealing).
