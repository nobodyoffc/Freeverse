...
- Plaintext IM-layer messages: `cipher` and `symkeyVersion` MUST NOT be set.
...
|`content`|plaintext payload appropriate to `contentType` (see FIMP1 §4 for TEXT, STREAM, VOICE, HAT formats)|
|`dataBase64`|present for STREAM, VOICE, etc.|
|`cipher`|MUST NOT be set|
...
Square messages are **plaintext** at the IM layer. `ImMessage.cipher` and `ImMessage.symkeyVersion` MUST NOT be set; receivers MUST reject any Square message that has either field set.
...
This document defines version 1 of the Square mode (FIMP3V1). Future versions MAY:
...
- **FIMP1V1** -- P2P mode (used for any direct member-to-member request/response).
- **FIMP2V1** -- Room mode (closed local-managed group).
- **FIMP4V1** -- Team mode (closed on-chain owner-managed group with encryption).
