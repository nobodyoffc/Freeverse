...
public class FcReplierHttp {
    ...
    public void Set0Success() {
        ...
        this.message = CodeMessage.Msg0Success;
    }

    public void Set1020Other(String message) {
        this.code = CodeMessage.Code1020OtherError;
        if(message==null)this.message = CodeMessage.Msg1020OtherError;
    ...
    public Long updateBalance(String sid, String api, long length, Jedis jedis, Double price) {
        ...
        if(fid.equals(paramsMap.get(ACCOUNT))){
            String minPay = paramsMap.get(FieldNames.MIN_PAYMENT);
        ...
        long newBalance;
        if(price==null)price = Double.parseDouble(paramsMap.get(FieldNames.PRICE_PER_K_BYTES));
        ...
        //update user balance
        long oldBalance = readHashLong(jedis, addSidBriefToName(sid, FieldNames.BALANCE), fid);
        newBalance = oldBalance - cost;
        if (newBalance < 0) {
            cost = oldBalance;
            jedis.hdel(addSidBriefToName(sid, FieldNames.BALANCE), fid);
            jedis.select(1);
            jedis.hdel(addSidBriefToName(sid, sessionName));
            jedis.select(0);
            newBalance = 0;
        } else
            jedis.hset(addSidBriefToName(sid, FieldNames.BALANCE), fid, String.valueOf(newBalance));

        //Update consume via balance
        if (via != null) {
            long oldViaBalance = readHashLong(jedis, addSidBriefToName(sid, FieldNames.CONSUME_VIA), via);
            long newViaBalance = oldViaBalance + cost;
            jedis.hset(addSidBriefToName(sid, FieldNames.CONSUME_VIA), via, String.valueOf(newViaBalance));
        }
    ...
    public void symSign(String sessionKey) {
        ...
        response.setHeader(CodeMessage.SignInHeader,sign);
    }
    ...
    public void replyOtherErrorHttp(String otherError, Object data, Jedis jedis) {
        replyHttp(CodeMessage.Code1020OtherError,otherError,data,jedis, null);
    }
    ...
    public String replyOtherJson(String otherError,Object data,Jedis jedis){
        return replyJson(CodeMessage.Code1020OtherError,otherError,data,jedis);
    }
