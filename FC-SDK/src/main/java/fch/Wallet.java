...
public class Wallet {
    ...
    public static String makeTx(BufferedReader br,byte[] priKey, String fidForOfflineSign, List<SendTo> sendToList, String opReturnStr, long cd, int maxCashes, ApipClient apipClient, ElasticsearchClient esClient){
        ...
        return makeTx(priKey,fidForOfflineSign,sendToList,opReturnStr,cd,maxCashes,apipClient,esClient);
    }
    ...
    public static String makeTx(byte[] priKey, String fidForOfflineSign, List<SendTo> sendToList, String opReturnStr, long cd, int maxCashes, ApipClient apipClient, ElasticsearchClient esClient) {
        ...
        while (iter.hasNext()) {
            ...
            // Long cd1 = cash.getCd();
            Long cd1 = ParseTools.cdd(cash.getValue(), cash.getBirthHeight(), bestHeight);
            cdSum += cd1;
    ...
    public static List<Nobody> checkNobodys(List<String> recipientList, ApipClient apipClient, ElasticsearchClient esClient) {
        ...
        if (apipClient != null) {
            ...
            if (result != null && result.hits() != null && !result.hits().hits().isEmpty()) {
                nobodyList.addAll(result.hits().hits().stream().map(Hit::source).collect(Collectors.toList()));
            }
        } else {
...
//    }
    ...
    private static CashListReturn getCdFromOneCash(String addrRequested, long cd, ElasticsearchClient esClient) throws IOException {
        ...
        cashListReturn.setTotal((long) result.hits().hits().size());
