...
public class Settings {
    ...
    public static final Double DEFAULT_WINDOW_TIME = 300000D;
    ...
    private  void initService(String accountAlias, ServiceType type, String apipAccountId, byte[] symKey, Configure config) {
        ApiAccount apiAccount = config.checkAPI(apipAccountId, mainFid, type,symKey, (Boolean) settingMap.get(SHARE_API));
        switch (type){
            case APIP,TALK,SWAP_HALL,MAP -> checkIfMainFidIsApiAccountUser(symKey,config,br,apiAccount, mainFid);
        }
    ...
    public void writeToFile(String fid, String sid){
        fileName = FileTools.makeFileName(fid, sid, SETTINGS, DOT_JSON);
    ...
    public void setting(byte[] symKey, BufferedReader br, @Nullable ServiceType serviceTypeOnlyForServer) {
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 5 -> config.updateApiAccount(config.chooseApiProviderOrAdd(config.getApiProviderMap(), apipClient),symKey,apipClient, (Boolean) settingMap.get(SHARE_API));
