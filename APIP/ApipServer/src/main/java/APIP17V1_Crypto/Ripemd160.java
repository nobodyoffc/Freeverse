...
@WebServlet(name = ApipApiNames.RIPEMD_160_HEX, value = "/"+ ApipApiNames.SN_17+"/"+ ApipApiNames.VERSION_1 +"/"+ ApipApiNames.RIPEMD_160_HEX)
public class Ripemd160 extends HttpServlet {
    ...
    protected void doRequest(HttpServletRequest request, HttpServletResponse response, AuthType authType, Settings settings) {
        ...
        if (!Hex.isHex32(text)) {
            replier.replyOtherErrorHttp("It is not a hex.", response);
            return;
        }
        byte[] textBytes = Hex.fromHex(text);
        replier.replySingleDataSuccessHttp(Hex.toHex(Hash.Ripemd160(textBytes)),response);
    }
}
