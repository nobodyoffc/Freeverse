...
import clients.talkClient.TalkSession;
...
class ServerTcpThread extends Thread {
    ...
    public ServerTcpThread(Socket socket, Map<String, Set<Socket>> fidSocketsMap, byte[] dealerPriKey, long price, Map<String,Long> nPriceMap, Service service, Settings settings, Map<String, byte[]> fidSessionKeyMap) {
        this.socket = socket;
...
//                updateUserBalance();
                ...
                socket.close();
                synchronized (TalkTcpServer.fidSocketsMap) {
                    TalkTcpServer.fidSocketsMap.get(userFid).remove(socket);
                }
                System.out.println(userFid + " disconnected. Active user number: " + TalkTcpServer.fidSocketsMap.size());
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    @Nullable
    private TalkSession getSession(byte[] keyName, DataOutputStream dos, JedisPool jedisPool) {
        TalkSession session;
        try(Jedis jedis = this.jedisPool.getResource()){
            session = TalkSession.getSessionFromJedisWithKeyName(Hex.toHex(keyName),sid,jedis);
            if(session==null){
                replyError(ReplyCodeMessage.Code1023MissSessionKey, null, null, dos);
                return null;
            }
...
//        if (timeOut()) return;
        ...
        try (Jedis jedis = jedisPool.getResource()) {
            ...
            if(requestBody.getMode()==null || requestBody.getMode().equals(RequestBody.SignInMode.NORMAL))
                session = TalkSession.getSessionFromJedisWithFid(talkUnitRequest.getFrom(),sid,jedis);

            if(session==null){
                session = TalkSession.makeNewSession(sid,jedis,talkUnitRequest.getFrom(),Hex.toHex(userPubKey),sessionDays);
            }

            this.userFid = session.getFid();
            this.userPubKey = Hex.fromHex(session.getPubKey());

            if(reply0Success(session.toJson(),dos, dealerPriKey,null, nonce))
                addSocket();
        }
    }
...
//    }
    ...
    private boolean checkRequest(TalkUnit talkUnitRequest,RequestBody requestBody, DataOutputStream dos, Jedis jedis) {
            ...
            if (isBadBalanceTcp(jedis, TalkTcpServer.sid, talkUnitRequest.getFrom())) {
                String data = "Send at lest " + talkParams.getMinPayment() + " F to " + talkParams.getDealer() + " to buy the service #" + TalkTcpServer.sid + ".";
                replyError(ReplyCodeMessage.Code1004InsufficientBalance, data,nonce,dos);
                return false;
            }
            if (!TalkTcpServer.sid.equals(requestBody.getSid())) {
